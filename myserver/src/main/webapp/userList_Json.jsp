...
<script type="text/javascript" >
	...
	function userUpdate() {
		...
		$('#btnUpdate').on('click',function(){
			...
			$.ajax({ 
			    ...
			    contentType : 'application/json', // 요청파라미터가 json -> 타입이 PUT일 경우, json타입으로만 보낼 수 있음
			    data: JSON.stringify({ id, name, password, role }), // 필드 이름과 변수명이 같을 경우, 변수명만 입력해도 됨 => json 축약형
			    //data: $("#form1").serialize() // 질의문자열. 제이쿼리 사용 시 serialize메서드 사용하여 간단하게 작업도 가능
			}).done( function(data) { 
			        userList();
	...
	function userInsert(){
		...
		$('#btnInsert').on('click',function(){
			...
			$.ajax({ 
			    url: conPath + '/users',  
			    type: 'POST',  
			    data: { id, name, password, role }, // query 문자열
			    dataType: 'json', 
	...
	function userList() {
		$.ajax({
			...
			type:'GET',
			dataType:'json'
		}).done(function(datas){ // success = done, 응답결과는 인수 datas에 들어있음
			$("tbody").empty();
			//$.each(datas,function(idx,item){ // forEach
			for(item of datas){
				$('<tr>')
				.data("id", item.id)
